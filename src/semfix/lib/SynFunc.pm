...
sub get_all_var {
	...
	foreach ( @{ $self->{_input} } ) {
		my $var_name = $_->{_name};
		push( @tmpout, "$func_name\_$var_name\_in" );
	}
	
	# @hoang get output vars
	foreach ( @{ $self->{_output} } ) {
		my $var_name = $_->{_name};
		push( @tmpout, "$func_name\_$var_name\_out" );
	}
...
sub get_decls {  #($ins_index, $io_index)
	...
	foreach ( @{ $self->{_input} } )
	{
		...
		my $new_var_name = "$func_name\_$var_name\_in\_ins$ins_index\_io$io_index";
		my $var_decl = Utility::getDeclare($new_var_name, $var_type);
		push( @decl, $var_decl );
		
		if ( $var_type eq "AIC" 
			|| $var_type eq "AII")
		{
			my $len_name = "$func_name\_$var_name\_in\_len\_ins$ins_index\_io$io_index";
			my $len_decl = Utility::getDeclare($len_name, "INT");
			push ( @decl, $len_decl );
		}
	}
	
	# @hoang output declaration
	foreach ( @{ $self->{_output} } ) {
		my $var_name  = $_->{_name};
		my $var_type = $_->{_type};
		
		$var_name = "$func_name\_$var_name\_out\_ins$ins_index\_io$io_index";
...
sub io_for_one_instance { # @hoang ($io_pair, $ins_index, $io_index, $symbol_vars)
	...
	my @in_vars  = @{ $io_pair->get_init_in_vars() };
	my @out_vars = @{ $io_pair->{_out_vars} };
	...
	if ( (scalar @out_vars) != (scalar @{ $self->{_output} }) ) {
		die "out_vars and syn_output_arr are mismatch!\n";
	}
	
	# @hoang connection for input
	foreach ( @{ $self->{_input} } ) {
		my $stmt_var = $_;
		my $stmt_var_name = $_->{_name};
		$stmt_var_name    = "$func_name\_$stmt_var_name\_in";
		my $stmt_var_type = $_->{_type};
		
		my $in_var = shift @in_vars;
		
		my $in_var_name = $in_var->{_name};
		my $in_value    = $in_var->{_value};
		my $in_type    = $in_var->{_type};
		$stmt_var_name = $stmt_var_name . "\_ins$ins_index\_io$io_index";
		
		my $var_constr_name;
		if ($in_type eq "CHAR")
		{
			$var_constr_name = Utility::getCHARRepresentation($stmt_var_name);
		}
		elsif ($in_type eq "INT")
		{
			$var_constr_name = Utility::getINTRepresentation($stmt_var_name);
		}
		elsif ($in_type eq "AIC"
				|| $in_type eq "AII")
		{
			$var_constr_name = $stmt_var_name;
		}
		else
		{
			die "Can not recognize in_type in SynFunc\n";
		}
		                      
		if ($in_value =~ /^\(\_\s+bv/ ) {
			# don't do anything
		}
		else {							# @hoang a symbolic value
			foreach( @symbol_vars ) {
				my $var_name = $_->{_name};
				DEBUG_PRINT("VAR_NAME: $var_name");
				unless ($in_type eq "AIC" || $in_type eq "AII") {
					$in_value =~ s/(\s+)$var_name(\s+)/$1$var_name\_io$io_index$2/g;
				}
			}
			if ($in_type eq "AIC" || $in_type eq "AII") {  #@hoang change array name
				$in_value =~ s/$in_var_name/$stmt_var_name/g;
			}
		}
		push( @constr, "(assert (= $var_constr_name $in_value))\n" );
		if ($in_type eq "AIC"
			|| $in_type eq "AII")		# TODO test this code
		{
			my $in_size     = $in_var->{_arr_size};
			if ($in_size == -1) {
				die "ARRAY SIZE is -1 in io_for_one_instance\n";
			}
			my $size_constr = "(_ bv$in_size 32)";
			
			$stmt_var_name     = $_->{_name};
			my $stmt_size_name = "$func_name\_$stmt_var_name\_in\_len";
			$stmt_size_name    = $stmt_size_name . "\_ins$ins_index\_io$io_index";
			
			my $stmt_size_value = Utility::getINTRepresentation($stmt_size_name);
			#push (@constr, "(assert (= $stmt_size_value $size_constr))\n");  FIXME: temporary do not impose any constraint on length
		}
	}
	
	# @hoang connection for output
	foreach ( @{ $self->get_all_output() } ) {
		
		my $out_var = shift @out_vars;
		my $out_name = $out_var->{_name};
		
		my $syn_var_type = $out_var->{_type};
		my $syn_var_name = "$_\_ins$ins_index\_io$io_index";
		$out_name = "$out_name\_io$io_index";
		
		my $syn_var_constr;
		my $out_value;
		if ($syn_var_type eq "Bool")
		{
			$syn_var_constr = $syn_var_name;
			$out_value	    = $out_name;
		}
		elsif ($syn_var_type eq "CHAR")
		{
			$syn_var_constr = Utility::getCHARRepresentation($syn_var_name);
			$out_value	    = Utility::getCHARRepresentation($out_name);
		}
		elsif ($syn_var_type eq "INT")
		{
			$syn_var_constr = Utility::getINTRepresentation($syn_var_name);          
		    $out_value		= Utility::getINTRepresentation($out_name);
		} 
		elsif ($syn_var_type eq "AIC" 
			|| $syn_var_type eq "AII")
		{
			$syn_var_constr = $syn_var_name;
			$out_value		= $out_name;
		...
		{
			die "Unrecognized syn_var_type in syn_func: $syn_var_type\n";
		}
